...
#First upload all csv files from the DB folder to workspace
#May need to zip the files first if importing is aa problem

#Next import as objects to global environment
#File names that change
hq_malt <- read.csv(file = "malt_hq_main_jun_2015.csv")
india_malt <- read.csv(file = "malt_hq_india_jan_jun_2015.csv")
#File names that shouldn't change
...
#dp_malt1 <- read.csv(file = "malt_dp1.csv")
#dp_malt2 <- read.csv(file = "malt_dp2.csv")
#dp_malt3 <- read.csv(file = "malt_dp3.csv")
#dp_malt4 <- read.csv(file = "malt_dp4.csv")
#dp_malt <- rbind(dp_malt1, dp_malt2, dp_malt3, dp_malt4)
...
dp_malt$form_month <- as.Date(dp_malt$form_month, format="%m/%d/%Y")
...
#Create table 1 by business unit
...
all_malt_user_month$start_month <- as.Date("2015-06-01")
all_malt_user_month$end_month <- as.Date("2015-06-01") 
