...
#Create form_date and form_month based on phone time
form_table$form_start_date <- as.Date(substr(form_table$time_start, 1, 10))

#Need to remove forms with time_start = NA (4.7% of forms don't have a valid time_start)
...
form_valid_start$form_start_month <- floor_date(form_valid_start$form_start_date, "month") 

#Create DP MALT
dp_malt_phone <- form_valid_start %>% group_by(domain_pk, user_pk, application_pk, form_start_month) %>% 
...
dp_malt_phone <- merge(dp_malt_phone, select(users, user_pk, user_id, username, email, is_web_user), 
                 by = "user_pk", all.x = T)
...
dp_malt_phone <- merge(dp_malt_phone, select(domain, domain_pk, name), by = "domain_pk", all.x = T)
names(dp_malt_phone)[names(dp_malt_phone) == "name"] = "domain"
...
dp_malt_phone <- merge(dp_malt_phone, select(app, application_pk, app_id, amplifies_workers, amplifies_project), 
                 by = "application_pk", all.x = T)

#Remove columns not in HQ MALT
dp_malt_phone <- select(dp_malt_phone, -c(application_pk, domain_pk, user_pk))


all_malt <- dp_malt_phone

#Add unique integer column
all_malt$unique_int <- 1:nrow(all_malt)

#Run lines 167-263 (user exclusions) in malt_hq_wam.R
...
all_malt_user_month <- all_malt %>% group_by(user_id, form_start_month) %>% 
...
all_malt_user_month <- arrange(all_malt_user_month, user_id, form_start_month)
all_malt_user_month <- all_malt_user_month %>% group_by(user_id) %>% 
  mutate(previous_active_months_rolling = seq_along(form_start_month)-1)
...
#Create Table 1
table_1_wam <- all_malt_user_month %>% group_by(form_start_month) %>% 
...
all_malt_table2 <- filter(all_malt_user_month, form_start_month >= start_month, 
                          form_start_month <= end_month)
