makeMonthly <- function(dat){
  ...
  # total unique cases that a mobile worker has interactions with (either registration or follow up) per month
  touched <- ddply(dat, .(user_id, month.index), function(x) length(unique(x$case_id)))
  colnames(touched)[3] <- c("total_case_touched")
  ...
  # merge data frames  
  if(length(temp_1)) merge_me <- list(user_0,user_1,user_2,user_3,user_4,user_5,user_6,user_7,user_8,user_9,temp_1, touched, res1_cast) else merge_me <- list(user_0,user_1,user_2,user_3,user_4,user_5,user_6,user_7,user_8,user_9, touched, res1_cast) # cum_case_registered could be empty
